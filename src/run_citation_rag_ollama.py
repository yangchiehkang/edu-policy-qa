...
OUTPUT_FILE = OUTPUT_DIR / "citation_rag_results.csv"
...
def call_ollama_generate_once(question, retrieved_chunks_text):
...
def call_ollama_generate_with_retry(question, retrieved_chunks_text):
    last_error = ""

    for attempt in range(1, GENERATION_RETRY_TIMES + 1):
        try:
            return call_ollama_generate_once(question, retrieved_chunks_text)
        except Exception as e:
            last_error = str(e)
            print(f"  Generation retry {attempt}/{GENERATION_RETRY_TIMES} failed: {last_error}", flush=True)
            time.sleep(2.0)
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        print(f"[{idx + 1}/{len(df)}] Citation RAG question_id={qid}", flush=True)
        ...
        try:
            ...
            answer = call_ollama_generate_with_retry(
                question=question,
        ...
        retrieved_sources = [
            get_source(item["chunk"]) for item in retrieved_items
        ]

        retrieved_pages = [
            get_page(item["chunk"]) for item in retrieved_items
        ]
    ...
    print()
    print(f"Saved Citation-constrained RAG results to: {OUTPUT_FILE}", flush=True)
